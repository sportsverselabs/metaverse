...
| 5 | **Creative Studio** | ✅ real | Dashboard-native video editor. Preview a draft render, reorder/trim clips, edit captions, generate a thumbnail, render a draft, **Request AI revision** (DeepSeek rewrites title/description/caption), automatic **compliance re-check**, and **Submit for review** (goes to Approvals → schedule → gated publisher). Renders are local drafts — **nothing publishes** until you approve + the separate gated publisher runs. |
| 6 | **Publishing** | functional | Shows real platform connection status from server-side adapters. YouTube now appears connected for private Platinum Clips uploads; TikTok and Instagram remain visibly pending until their credentials/app approvals are complete. Owner-approved/scheduled items only show enabled publish buttons for connected platforms. |
